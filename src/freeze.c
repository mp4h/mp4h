...
produce_frozen_state (const char *name)
{
  ...
  fprintf (file, "# This is a frozen state file generated by PACKAGE VERSION\n");
  fprintf (file, "V1\n");
...
{
  if (expected == '\n')
    MP4HERROR ((EXIT_FAILURE, 0, _("Expecting line feed in frozen file")));
  else
    MP4HERROR ((EXIT_FAILURE, 0, _("Expecting character `%c' in frozen file"),
              expected));
}
